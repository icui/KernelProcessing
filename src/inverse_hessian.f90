...
module inverse_hessian_sub
  ...
  subroutine get_sys_args(input_hess, input_model, output_hess, threshold_hess)
    ...
    if(input_hess == '' .or. input_model == '' .or. output_hess == '') then
      call exit_mpi("Usage: xinverse_hessian input_hess input_model output_hess threshold_hess")
    endif
...
program inverse_hessian
  ...
  ! vs
  call prepare_hessian(hess_vs, threshold_hess, invhess(:, :, :, :, 1))

  ! vp
  call prepare_hessian(hess_vp, threshold_hess, invhess(:, :, :, :, 2))

  ! eta
  call prepare_hessian(hess_eta, threshold_hess, invhess(:, :, :, :, 3))

  ! rho
  call prepare_hessian(hess_rho, threshold_hess, invhess(:, :, :, :, 4))
